	...
	tokens := []data.Token{
		{
			...
			Chains: []data.TokenChain{
				{
					ChainID:      "goerli",
					TokenType:    qtum.TokenTypeNative,
					BridgingType: data.BridgingTypeLP,
					AutoSend:     false,
				},
				{
					ChainID:      "goerli",
					TokenType:    qtum.TokenTypeErc20,
					BridgingType: data.BridgingTypeLP,
					AutoSend:     false,
				},
			},
...
func PrepareRedeemMultisig() (*types.Transaction, error) {
	...
	proxy = qtum.ChangeSigner(proxy, signer)
	redeemNativeFn := qtum.ExportRedeemNative(proxy)
...
func TestCheckTxDataAndSignSuccess(t *testing.T) {
	...
	checkTxFn := qtum.ExportCheckTxDataAndSign(proxy)
	_, signNum, err := checkTxFn(qtum.ExportBuildTxOpts()(sender), tx2, tx1.Data())
	require.NoError(t, err)
...
func TestCheckTxDataAndSignFail(t *testing.T) {
	...
	checkTxFn := qtum.ExportCheckTxDataAndSign(proxy)
	_, _, err = checkTxFn(qtum.ExportBuildTxOpts()(sender), tx2, tx1.Data())
	require.Error(t, err)
}
