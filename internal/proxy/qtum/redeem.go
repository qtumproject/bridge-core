...
func (p *evmProxy) sendTx(tx *ethTypes.Transaction, chain string) (interface{}, error) {
	...
	params, _, err := decodeTxParams(*abi, tx.Data())
	if err != nil {
		return nil, errors.Wrap(err, "failed to decode tx params")
	}

	rawSignatures := params[6].([][]byte)
	signatures := make([]string, len(rawSignatures))
	for i, rawSignature := range rawSignatures {
		signatures[i] = "0x" + common.Bytes2Hex(rawSignature)
	}
	body, err := json.Marshal(map[string]interface{}{
		"bridge_contract": p.bridgeContract.String(),
		"token_address":   params[0].(common.Address).String(),
		"amount":          params[1].(*big.Int).String(),
		"receiver":        params[2].(common.Address).String(),
		"tx_hash":         common.Hash(params[3].([32]byte)).String(),
		"tx_nonce":        params[4].(*big.Int).String(),
		"signatures":      signatures,
	})
	if err != nil {
		return nil, errors.Wrap(err, "failed to marshal body")
	}
	resp, err := http.Post(p.proxyUrl, "application/json", strings.NewReader(string(body)))
	if err != nil {
		return nil, errors.Wrap(err, "failed to send tx")
	}
	if resp.StatusCode != http.StatusOK {
		return nil, errors.New("failed to send tx")
	}
	...
	return encodeProcessedTx(common.HexToHash(result["tx_hash"].(string)), chain), errors.Wrap(err, "failed to send tx")
}
